...
| `install` | Your install command. **Runs even with `skipChecks`**. Served from a package cache kept per repository, and retried with fewer parallel fetches while the failure looks like the network's ([ADR 0016](adr/0016-stop-asking-the-registry.md)) |
